...
function AdminBookings() {
  ...
  const handleRefund = async (booking) => {
    ...
    const daysDifference = (fromDate - today) / (1000 * 3600 * 24);
    ...
    Swal.fire({
      ...
      text: `Refund Amount: BDT ${refundAmount}`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axios.post("http://localhost:7700/api/bookings/refundBooking", {
            bookingId: booking._id,
            refundAmount,
          });

          // Add notification
          await axios.post("http://localhost:7700/api/notifications/addNotification", {
            userid: booking.userid,
            message: "Booking Refund",
            body: `Your booking has been refunded. Amount: BDT ${refundAmount}`,
          });
                      ...
                      bookings.map((booking, index) => (
                        <tr key={index}>
                          ...
                          <td className='flex gap-2'>
                            {booking.status === "booked" && !booking.isRefund && (
                              <button 
                                onClick={() => handleRefund(booking)} 
                                className="w-[35px] h-[35px] rounded-full border flex justify-center items-center bg-red-600 hover:bg-red-700 duration-300">
